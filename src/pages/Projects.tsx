...
import Img1 from '../assets/Projects1.svg';
import Sb from '../assets/loader.webp';
import Mh from '../assets/loader.webp';
import Ht from '../assets/loader.webp';
import Hc from '../assets/loader.webp';
import Ku from '../assets/loader.webp';
import Navbar from '../components/Navbar';

const ProjectBox: React.FC<{ title: string; image: string }> = ({ title, image }) => {
  return (
    <div className="group bg-green-100 h-60 w-96 rounded-3xl border-4 border-green-600 shadow-green-300 transform shadow-md p-4 transition-all duration-500 delay-50 hover:scale-105 hover:bg-opacity-45 relative overflow-hidden">
      <img src={image} alt={title} className="w-full h-full object-cover rounded-3xl" />
      <div className="text-3xl text-center mb-16 absolute inset-0 flex flex-col items-center justify-end opacity-0 transition-opacity duration-300 p-4 rounded-md group-hover:opacity-100">
        {title}
